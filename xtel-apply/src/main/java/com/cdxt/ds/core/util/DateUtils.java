...
public class DateUtils extends PropertyEditorSupport {
	...
	/**
	 * <p>方法名称: getEndDate</p>
	 * <p>方法描述: 设置时间为该天的最后一秒 23:59:59</p>
	 * @param endDate
	 * @return Date
	 */
	 ...
	 * 
	 * @Title: gMtToDate
	 * @Description:GPT字符串转换为date类型
	 * @param
	 * @return
	 */
	public static Date gMtToDate(String strDate){
		Date date =new Date();

		//	SimpleDateFormat sdf = new SimpleDateFormat("EE MMM dd yyyy HH:mm:ss z",Locale.US);
		date.parse(strDate);
	 ...
	 * @return
	 */
	public static List<String> findDatesStr(String dBegin,String dEnd) {
		List<String> lDate = new ArrayList<String>();
		Date satrtDate=str2Date(dBegin,date_sdf);
		Date endDate=str2Date(dEnd,date_sdf);
		lDate.add(dBegin);
		Calendar calBegin = Calendar.getInstance();
		// 使用给定的 Date 设置此 Calendar 的时间
		calBegin.setTime(satrtDate);
		//	Calendar calEnd = Calendar.getInstance();
		// 使用给定的 Date 设置此 Calendar 的时间
		...
		while (endDate.after(calBegin.getTime()))  {
			// 根据日历的规则，为给定的日历字段添加或减去指定的时间量
			calBegin.add(Calendar.DATE, 1);
			lDate.add(date2Str(calBegin.getTime(),date_sdf));
		}
		return lDate;
	}
	...
	public static Date getDateOfFirstTime(){
		Calendar calendar = Calendar.getInstance();
	...
	/**
	 * 
	 * @Title: convertTimeToLong
	 * @author wangxiaolong
	 * @Description:转换时间日期格式字串为long型	 
	 * @param
	 * @return
	 */
	public static Long str2Long(String time) {
		Date date=null;
		try {
			date =str2Date(time,datetimeFormat);
			if(date==null){
				return 0L;
			}else{
				return date.getTime();
